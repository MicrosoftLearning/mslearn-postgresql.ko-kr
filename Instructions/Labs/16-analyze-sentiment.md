...
Margie's Travel을 위해 빌드하는 AI 기반 앱의 일환으로 개별 리뷰의 감정과 선택한 임대 부동산에 대한 모든 리뷰의 전반적인 감정에 대한 정보를 사용자에게 제공하고자 합니다. 이를 위해 Azure Database for PostgreSQL 유연한 서버에서 `azure_ai` 확장을 사용하여 감정 분석 기능을 데이터베이스에 통합합니다.
...
4. 또한 쿼리의 출력을 올바르게 처리할 수 있도록 함수가 반환하는 데이터 형식의 구조를 이해해야 합니다. `sentiment_analysis_result` 형식을 검사하기 위해 다음 명령을 실행합니다.
...
3. 아래 쿼리를 실행하여 동일한 업데이트 작업을 수행해 보겠습니다. 하지만 이번에는 `reviews` 테이블의 댓글을 10개씩 일괄 처리(허용되는 최대 일괄 처리 크기)하도록 보내고 성능 차이를 평가합니다.
    ...
    이 쿼리는 좀 더 복잡하지만 두 개의 CTE를 사용하여 훨씬 성능이 향상됩니다. 이 쿼리에서 첫 번째 CTE는 일괄 처리하는 리뷰 댓글의 감정을 분석하고, 두 번째 CTE는 각 행별로 서수 위치 및 ``sentiment_analysis_result` 기반의 `id`을(를) 포함하는 새 테이블로 `sentiment_analysis_results`의 결과 테이블을 추출합니다. 그런 다음 update 문에서 두 번째 CTE를 사용하여 데이터베이스에 값을 쓸 수 있습니다.

4. 다음으로 쿼리를 실행하여 업데이트 결과를 관찰하고, **부정적인** 감정을 가진 리뷰를 가장 부정적인 감정부터 시작하여 검색합니다.
