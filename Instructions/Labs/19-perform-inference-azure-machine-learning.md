...
9. 배포 플라이아웃 메뉴에서 **가상 머신**을 **Standard_DS2_v2**와 같이 설정하고 **인스턴스 수**를 1로 설정합니다. **배포** 버튼을 선택합니다. 배포 프로세스에는 가상 머신 프로비전과 Docker 컨테이너로 모델을 배포하는 과정이 포함되므로 배포를 완료하는 데 몇 분 정도 걸릴 수 있습니다.
